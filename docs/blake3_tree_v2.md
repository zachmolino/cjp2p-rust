...
below) so the format has a description outside the code. Where the node's current
checks fall short of these, "Where the code differs today" says so.
...
only says it is in `((n-1) * 4096, n * 4096]`, and the last block's own chaining
value fixes it.
...
- `check_tree_v2_data` does not compare copied-up entries (check 4) and does not
  check the root merge (check 5).
- `n_leaves_from_tree_v2_eof` returns `None` below 128 bytes, so a one-block tree
  (96 bytes) is rejected and content of 4096 bytes or less cannot be verified
  through a tree; the per-block check also compares that single block with its
  leaf instead of the root.

Both are addressed in separate changes.
